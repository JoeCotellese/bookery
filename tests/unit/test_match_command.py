...
class TestMatchCommand:
    ...
    def test_match_single_file_quiet(self, sample_epub: Path, tmp_path: Path) -> None:
        ...
        with (
            patch("bookery.cli.commands.match_cmd._create_provider") as mock_provider_fn,
        ):
    ...
    def test_match_reports_skipped(self, sample_epub: Path, tmp_path: Path) -> None:
        ...
        with patch("bookery.cli.commands.match_cmd._create_provider") as mock_provider_fn:
            mock_provider = MagicMock()
    ...
    def test_match_output_dir_default(
        ...
        with (
            patch("bookery.cli.commands.match_cmd._create_provider") as mock_provider_fn,
            patch("bookery.core.pipeline.apply_metadata_safely") as mock_apply,
        ):
            mock_provider = MagicMock()
            mock_provider.search_by_isbn.return_value = []
            mock_provider.search_by_title_author.return_value = [candidate]
...
class TestThresholdOption:
    ...
    def test_threshold_option_passed_to_review(self, sample_epub: Path, tmp_path: Path) -> None:
...
class TestResumeOption:
    ...
    def test_resume_skips_existing_output(self, sample_epub: Path, tmp_path: Path) -> None:
    ...
    def test_resume_shows_skip_count(self, sample_epub: Path, tmp_path: Path) -> None:
    ...
    def test_no_resume_processes_all(self, sample_epub: Path, tmp_path: Path) -> None:
    ...
    def test_resume_handles_collision_suffixes(self, sample_epub: Path, tmp_path: Path) -> None:
    ...
    def test_resume_finds_files_via_manifest(self, sample_epub: Path, tmp_path: Path) -> None:
    ...
    def test_all_processed_shows_message(self, sample_epub: Path, tmp_path: Path) -> None:
