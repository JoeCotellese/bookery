...
def test_convert_pdf_to_epub_returns_epub_path(tmp_path: Path) -> None:
    ...
    with patch("bookery.cli._pdf_support.convert_pdf", return_value=result) as mocked:
...
def test_convert_pdf_to_epub_prints_warnings(tmp_path: Path) -> None:
    src = tmp_path / "src.pdf"
    epub = tmp_path / "src.epub"
    result = PdfConvertResult(source=src, epub_path=epub, warnings=("something fishy",))
